...
extern "C" map * __untracer_find_breakpoint(map *hash_map, uintptr_t key)
{
    map *entry = NULL;
...
extern "C" void __untracer_add_breakpoint(map **hash_map, uintptr_t key, breakpoint bp)
{
...
extern "C" void __untracer_make_writable(uint64_t addr)
{
...
extern "C" void __untracer_sigtrap_handler(int sig, siginfo_t *info, void *ctx) {
...
extern "C" void __untracer_crash_handler(int sig) {
    printf("crash %d\n", sig);
    exit(1);
}

extern "C" void __untracer_setup_handler(void) {
    struct sigaction sa;
...
extern "C" int __untracer_read_csv(const char * csv) {
